...
### Codex Model
Always use `Codex-sonnet-4-5-20250929` as the default model for Codex API calls.
...
### What We Focus On
- LLM API integration patterns (Codex, OpenAI, etc.)
...
- `POST /projects/{id}/chats/{chat_id}/messages` - Send message (calls Codex API)
...
**Categories:** `ai` (Codex, OpenAI, ElevenLabs, Gemini, VEO), `storage` (Pinecone), `utility` (Tavily, Google OAuth, Webshare), `integrations` (Notion, Jira)
...
Token counting uses **tiktoken** (local) for speed, with Codex API available for exact counts when needed.
...
tiktoken uses `cl100k_base` encoding which closely matches Codex's tokenizer (within ~5% accuracy - good enough for chunking).

## Web Agent Architecture

**Agentic loop pattern** for URL content extraction. Loop: Codex calls tools → execute → return result → repeat until termination.

**Tool Types**:
- **Server Tools**: Codex handles execution (web_fetch, web_search) - results come in response
...
### Tool Use Loop
```
User message → Codex API (with search_sources, store_memory tools)
    ↓
...
**Key Point**: Codex passes `source_id` to search, but receives `chunk_id` in results. Citations must use `chunk_id`, NOT `source_id`.
...
**Flow**:
```
Codex response: "Information here [[cite:abc123_page_5_chunk_2]]"
       ↓
