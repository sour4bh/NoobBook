...
This section describes the Claude-API integration pattern NoobBook uses: configuration loading, path management, the API call, and response parsing. Modules that currently live under `ai_services/`, `ai_agents/`, and `tool_executors/` follow it. Those bucket names describe where these modules live today during the structure migration — they are legacy/migration sources per `STRUCTURE.md`, not preferred homes for new work. When adding a new Claude-API integration, keep the steps below and pick a domain-owned destination per `STRUCTURE.md` and the `NBB-104` charter.
