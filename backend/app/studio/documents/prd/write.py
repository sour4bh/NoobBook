...
from app.studio.documents.prd.tools.binding import bind_prd_tools
...
class PRDWriter:
    ...
    def generate_prd(
        ...
        logger.warning("PRD agent completed without final section")
        error_result = {
            "success": False,
            "error_message": "Agent completed without finalizing the PRD",
            "iterations": self._iteration_count(result),
    ...
    def _terminating_tool_result(self, result: Any) -> Optional[Dict[str, Any]]:
        for tool_result in reversed(result.tool_results):
            if tool_result.name == "write_prd_section" and isinstance(tool_result.content, dict):
