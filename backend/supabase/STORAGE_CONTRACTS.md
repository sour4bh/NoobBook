...
|---|---|---|---|---|---|---|---|
| `raw-files` | false | 100 MB | `migrations/00002_storage_buckets.sql`; owner-prefix reconciliation in `00021_storage_owner_paths.sql` | `auth.uid()::text == (storage.foldername(name))[1]` (per-op) | Same owner-prefix policy in `init.sql` | `_build_source_path(project_id, source_id, filename)` in `backend/app/providers/supabase/storage.py` | `sources/` |
| `processed-files` | false | 100 MB | `migrations/00002_storage_buckets.sql`; `00021_storage_owner_paths.sql` | Same `auth.uid()` shape as `raw-files` | Same owner-prefix policy | `_build_source_path(project_id, source_id, filename)` | `sources/` |
| `chunks` | false | 10 MB | `migrations/00002_storage_buckets.sql`; chunk helper in `00021_storage_owner_paths.sql` | Same `auth.uid()` shape | Same owner-prefix policy | `_build_source_path(project_id, source_id, chunk_id)` (same helper, called with chunk filenames) | `sources/` (indexing slice) |
| `studio-outputs` | false | 500 MB | `migrations/00002_storage_buckets.sql`; path helper update in `00021_storage_owner_paths.sql` | Same `auth.uid()` shape | Same owner-prefix policy | `_build_studio_path(project_id, job_type, job_id, filename)` and `_build_ai_image_path(project_id, filename)` | `studio/` |
...
|---|---|---|---|---|
| `raw-files` | `{user_id}/{project_id}/{source_id}/{filename}` | `generate_raw_file_path(user_id, project_id, source_id, filename)` defines the same shape in 00002 | `_build_source_path(project_id, source_id, filename)` | Existing unprefixed objects are backfilled by `00021_storage_owner_paths.sql`. |
| `processed-files` | `{user_id}/{project_id}/{source_id}/{source_id}.txt` | `generate_processed_file_path` in 00002 defines the same shape | `_build_source_path(project_id, source_id, filename)` | Existing unprefixed source metadata paths are backfilled by `00021_storage_owner_paths.sql`. |
| `chunks` | `{user_id}/{project_id}/{source_id}/{chunk_id}.txt` | `generate_chunk_file_path` in 00021 defines this shape | `_build_source_path(project_id, source_id, chunk_id)` | Chunk id format itself remains unchanged. |
| `studio-outputs` | `{user_id}/{project_id}/studio/{job_type}/{job_id}/{filename}` and `{user_id}/{project_id}/ai-images/{filename}` | `generate_studio_output_path` and `generate_ai_image_path` in 00021 define these shapes | `_build_studio_path(project_id, job_type, job_id, filename)` and `_build_ai_image_path(project_id, filename)` | Existing unprefixed objects are backfilled by `00021_storage_owner_paths.sql`. |
...
| `studio-outputs` | `GET /api/v1/projects/{project_id}/studio/{category}/...` category routes in `backend/app/api/studio/` generate signed URLs via `storage_service.get_studio_signed_url`/`get_studio_public_url`, or stream bytes through backend routes | Studio generators write via `storage_service.upload_studio_file` / `upload_studio_binary`; CSV analysis writes generated images through `upload_ai_image` | Project guard plus owner-prefixed storage keys. `studio_jobs` has no table RLS (see `migrations/OWNERS.md`), so route-level project guard remains required before signed URL issuance or byte streaming. |
