...
import app.providers.anthropic.usage
...
class TestResponseTypeChecks:
    ...
    def test_is_end_turn_true(self):
        assert app.providers.anthropic.response_parser.is_end_turn({"stop_reason": "end_turn"}) is True

    def test_is_end_turn_false_for_tool_use(self):
        assert app.providers.anthropic.response_parser.is_end_turn({"stop_reason": "tool_use"}) is False

    def test_is_end_turn_false_for_missing(self):
        assert app.providers.anthropic.response_parser.is_end_turn({}) is False

    def test_is_tool_use_true(self):
        assert app.providers.anthropic.response_parser.is_tool_use({"stop_reason": "tool_use"}) is True

    def test_is_tool_use_false_for_end_turn(self):
        assert app.providers.anthropic.response_parser.is_tool_use({"stop_reason": "end_turn"}) is False

    def test_get_stop_reason(self):
        assert app.providers.anthropic.response_parser.get_stop_reason({"stop_reason": "max_tokens"}) == "max_tokens"

    def test_get_stop_reason_missing(self):
        assert app.providers.anthropic.response_parser.get_stop_reason({}) == ""
...
class TestExtractText:
    ...
    def test_dict_format(self):
        ...
        assert app.providers.anthropic.response_parser.extract_text(resp) == "hello"

    def test_sdk_format(self):
        resp = {"content_blocks": [_sdk_text("world")]}
        assert app.providers.anthropic.response_parser.extract_text(resp) == "world"
    ...
    def test_mixed_blocks_only_text(self):
        ...
        assert app.providers.anthropic.response_parser.extract_text(resp) == "I'll search\nFound it"

    def test_empty_content_blocks(self):
        assert app.providers.anthropic.response_parser.extract_text({"content_blocks": []}) == ""

    def test_missing_content_blocks(self):
        assert app.providers.anthropic.response_parser.extract_text({}) == ""

    def test_multiple_text_blocks_joined(self):
        resp = {"content_blocks": [_sdk_text("a"), _sdk_text("b"), _sdk_text("c")]}
        assert app.providers.anthropic.response_parser.extract_text(resp) == "a\nb\nc"
...
class TestExtractCitations:
    ...
    def test_dict_citations(self):
        ...
        result = app.providers.anthropic.response_parser.extract_citations(resp)
        assert len(result) == 1
    ...
    def test_no_citations(self):
        ...
        assert app.providers.anthropic.response_parser.extract_citations(resp) == []
...
class TestExtractToolUseBlocks:
    ...
    def test_dict_format(self):
        ...
        blocks = app.providers.anthropic.response_parser.extract_tool_use_blocks(resp)
        assert len(blocks) == 1
    ...
    def test_filter_by_name(self):
        ...
        blocks = app.providers.anthropic.response_parser.extract_tool_use_blocks(resp, tool_name="search")
        assert len(blocks) == 2
        assert all(b["name"] == "search" for b in blocks)

    def test_no_match(self):
        resp = {"content_blocks": [_dict_text("hello")]}
        assert app.providers.anthropic.response_parser.extract_tool_use_blocks(resp) == []
    ...
    def test_extract_tool_inputs(self):
        ...
        inputs = app.providers.anthropic.response_parser.extract_tool_inputs(resp, "search")
...
class TestServerToolExtraction:
    ...
    def test_extract_server_tool_use_sdk(self):
        ...
        blocks = app.providers.anthropic.response_parser.extract_server_tool_use_blocks(resp)
        assert len(blocks) == 1
    ...
    def test_extract_server_tool_use_dict(self):
        ...
        blocks = app.providers.anthropic.response_parser.extract_server_tool_use_blocks(resp, tool_name="web_fetch")
        assert len(blocks) == 1

    def test_has_server_tool_use(self):
        resp = {"content_blocks": [_sdk_server_tool_use("s1", "web_search", {})]}
        assert app.providers.anthropic.response_parser.has_server_tool_use(resp) is True

    def test_has_no_server_tool_use(self):
        resp = {"content_blocks": [_dict_text("no tools")]}
        assert app.providers.anthropic.response_parser.has_server_tool_use(resp) is False
    ...
    def test_extract_server_tool_results_dict(self):
        ...
        results = app.providers.anthropic.response_parser.extract_server_tool_results(resp)
    ...
    def test_extract_server_tool_results_filter(self):
        ...
        results = app.providers.anthropic.response_parser.extract_server_tool_results(resp, result_type="web_fetch_tool_result")
        assert len(results) == 1
...
class TestBuildToolResult:
    ...
    def test_basic_result(self):
        results = app.providers.anthropic.content.build_tool_result_content([
    ...
    def test_build_single_tool_result(self):
        results = app.providers.anthropic.content.build_single_tool_result("t1", "done")
        assert len(results) == 1
        assert results[0]["tool_use_id"] == "t1"
        assert results[0]["content"] == "done"

    def test_build_single_tool_result_error(self):
        results = app.providers.anthropic.content.build_single_tool_result("t1", "fail", is_error=True)
        assert results[0]["is_error"] is True
...
class TestSerializeAnthropicObject:
    ...
    def test_none(self):
        assert app.providers.anthropic.content._serialize_anthropic_object(None) is None

    def test_string(self):
        assert app.providers.anthropic.content._serialize_anthropic_object("hello") == "hello"

    def test_int(self):
        assert app.providers.anthropic.content._serialize_anthropic_object(42) == 42

    def test_bool(self):
        assert app.providers.anthropic.content._serialize_anthropic_object(True) is True

    def test_list(self):
        assert app.providers.anthropic.content._serialize_anthropic_object([1, "a"]) == [1, "a"]

    def test_dict(self):
        assert app.providers.anthropic.content._serialize_anthropic_object({"k": "v"}) == {"k": "v"}

    def test_object_with_dict(self):
        obj = SimpleNamespace(type="text", text="hello")
        result = app.providers.anthropic.content._serialize_anthropic_object(obj)
    ...
    def test_nested_object(self):
        ...
        result = app.providers.anthropic.content._serialize_anthropic_object(outer)
...
class TestSerializeContentBlocks:
    ...
    def test_sdk_text_block(self):
        ...
        result = app.providers.anthropic.content.serialize_content_blocks(blocks)
        assert result == [{"type": "text", "text": "hello"}]
    ...
    def test_sdk_text_with_citations(self):
        ...
        result = app.providers.anthropic.content.serialize_content_blocks([block])
...
class TestTokenUsageAndModel:
    ...
    def test_get_token_usage(self):
        ...
        usage = app.providers.anthropic.usage.get_token_usage(resp)
        assert usage == {"input_tokens": 100, "output_tokens": 50}

    def test_get_token_usage_missing(self):
        assert app.providers.anthropic.usage.get_token_usage({}) == {"input_tokens": 0, "output_tokens": 0}

    def test_get_model(self):
        assert app.providers.anthropic.usage.get_model({"model": "claude-sonnet-4-6"}) == "claude-sonnet-4-6"

    def test_get_model_missing(self):
        assert app.providers.anthropic.usage.get_model({}) == ""
