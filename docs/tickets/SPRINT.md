...
| `NBB-107` | Policy and contracts | Ready | unassigned |  |  |  |  | Deps merged: `NBB-103` at `df44200`. Write scope: `backend/tests/` (primary — new auth/permissions test files); `backend/app/api/`, `backend/app/services/auth/`, `backend/app/utils/auth_middleware.py` only for minimal test-seam adjustments. No refactory. Collisions: none against currently ready tickets when runtime-code edits are kept to minimal test seams. |
...
| `NBB-203` | Policy and contracts | Ready | unassigned |  |  |  |  | Deps merged: `NBB-103` at `df44200`. Write scope: `backend/app/services/ai_agents/csv_analyzer_agent.py`, `backend/app/services/tool_executors/analysis_executor.py`, `backend/app/services/tools/analysis_agent/`, `backend/tests/` (new security/analysis test files). No refactory. Collisions: none against other currently ready tickets; test files must use narrow fixture names to stay disjoint from `NBB-107` tests. |
...
| `NBB-205` | Policy and contracts | Ready | unassigned |  |  |  |  | Deps merged: `NBB-104` at `b12bf50`. Write scope: backend contract docs/schemas (new files), `frontend/src/lib/api/` (docs/notes only — contract preservation, not API rewrite), `backend/app/api/` (inspection-only — no edits to existing `api/*.py`). `DEFERRED.md` already resolved in `NBB-108A`; new deferrals only if discovered. No refactory. Collisions: none when `backend/app/api/` is treated as inspection-only. |
| `NBB-206` | Policy and contracts | Ready | unassigned |  |  |  |  | Deps merged: `NBB-104` at `b12bf50`. Write scope: `backend/app/providers/`, `backend/app/connectors/`, backend package charters (new per-root overlays analogous to `NBB-204`'s for providers/ and connectors/ only), structure/dependency docs. No refactory. Collisions: serialize after `NBB-208A` — NBB-208A edits provider/connector config reload hooks; NBB-206 creates provider/connector charters. Running them in parallel risks `backend/app/providers/` and `backend/app/connectors/` directory collisions during bootstrap edits. |
