...
As of 2026-04-25, end of Phase 4/5 batch 8:

- Branch: `main` @ `9667275`
- Merged progress: 33 of 59 tasks merged (Phase 1-3 complete; Phase 4 batch 1: NBB-208B, NBB-401, NBB-704A; batch 2: NBB-209D, NBB-209B; batch 3: NBB-209E, NBB-207B; batch 4: NBB-207C, NBB-210; batch 5: NBB-501A, NBB-209A; batch 6: NBB-501B, NBB-402; batch 7: NBB-502, NBB-201; batch 8: NBB-202A, NBB-503)
- Batch 9 dispatched: NBB-209C, NBB-504, NBB-505, NBB-506. NBB-507/202B/602/603/705C held for batch 10 to validate 3-wide studio parallelism + avoid `auth/__init__.py` collision (202B vs 209C) and `lib/contexts/` overlap (602 vs 603).
...
| `NBB-209C` | Stores and mechanical moves | In progress | nbb-worker (batch 9) | TBD by harness | pending | | | Baseline `main@9667275`. Deps merged: `NBB-201` at `6aeeffc`. Auth user/password store move. Pre-auth Cat A/B/C + Blueprint-init. `get_user_service()` factory partially protects (basename `user_service.py` matches but consumers use factory call, not bare singleton). 4 known import sites: `cost_tracking.py:300/357`, `api/settings/users.py:15`, `data_services/__init__.py:27`. Unblocks NBB-705A. |
...
| `NBB-705A` | Verification and cleanup | Blocked | unassigned |  |  |  |  | Blocked on `NBB-209C` only (NBB-201 merged at `6aeeffc`). Drain auth utilities. NBB-201 worker deferred broader data-services policy removal here. |
...
| `NBB-504` | Studio | In progress | nbb-worker (batch 9) | TBD by harness | pending | | | Baseline `main@9667275`. Deps merged: `NBB-503` at `4980de5`. Items: blog→`write.py`, business_report→`write.py`, prd→`write.py`, presentation→`compose.py`. Verb table from NBB-502 LAYER_MAP. METHOD-rename to `dispatch(...)` (NBB-503 P1 lesson). |
| `NBB-505` | Studio | In progress | nbb-worker (batch 9) | TBD by harness | pending | | | Baseline `main@9667275`. Deps merged: `NBB-503` at `4980de5`. Items: ad→`create.py`, email→`write.py`, infographic→`create.py`, strategy→`plan.py`, social_post→`write.py`. METHOD-rename to `dispatch(...)`. |
| `NBB-506` | Studio | In progress | nbb-worker (batch 9) | TBD by harness | pending | | | Baseline `main@9667275`. Deps merged: `NBB-503` at `4980de5`. Items: component→`build.py`, flow_diagram→`build.py`, logo→`create.py`, wireframe→`draw.py` (website was pilot). `logo_utils.py` ownership decision: stays studio-design-owned. Brand stores untouched (already at `brand/` per NBB-209D). METHOD-rename to `dispatch(...)`. |
